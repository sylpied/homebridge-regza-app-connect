...
このため、DBR-M590のHomeKit読み取り状態は楽観的に管理します。純正リモコンなど別経路の電源操作を確実に検出できる読み取り専用APIは確認できていません。一方、スタートメニュー`46`はスタンバイ中でもDBRを起動できることを実機確認しました。プラグインはON要求でDBRへ`46`を送り、起動待ちを入れずに選択した連動TVへ専用ONを送ります。OFF要求では、連動TVがOFFと確認できる時だけ`46`で既知のON状態へ揃え、既定10秒待機してからトグル`12`を送ります。連動TVがONの時は、HDMI切替などで視聴を乱さないよう`12`だけを送ります。さらに、連動TVの確定したON→OFF遷移を検出すると、既定5秒後に`46`→10秒→`12`を自動実行します。起動時にTVが既にOFFの場合は実行しません。最初の5秒または`46`後の10秒待機中にTVがONへ戻れば、最後の`12`をキャンセルします。DBRごとの直列化により、新しい電源要求は古い未完了シーケンスを無効化します。

DBR用チャンネルUP `1e`／DOWN `1f`はON中のチャンネル切り替えに使用できますが、スタンバイからは起動しません。純正APKの機種別キー表も確認し、TV用の専用ON `40BF7E`／OFF `40BF7F`に対応するDBR列は空欄でした。短縮候補`7e`／`7f`もDBR-M590実機では無反応でした。
...
連動TVのON→OFFに伴う収束処理は、HomeKit上のDBR表示がON／OFFのどちらでも実行します。DBRの表示状態は読み取り値ではなく楽観状態だからです。
