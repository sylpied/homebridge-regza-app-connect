...
電源状態は楽観的に管理します。純正リモコンなど別経路で電源を変更した場合はHomeKit表示と一致しないことがあります。TCP 1048、SSDP、UPnP SOAPのON/OFF比較はいずれも完全に同一で、待機中もネットdeナビ／DLNAサーバーが稼働するため、現時点では安全な読み取り専用判定方法を確認できていません。詳細は[プロトコル資料](docs/PROTOCOL.ja.md)を参照してください。
