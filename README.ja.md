...
通常視聴中は、上／下でチャンネルUP／DOWN、左／右のどちらでも地デジとBSをトグル切替します。CSまたはHDMI表示中の最初のトグルは地デジへ戻します。地デジ／BS／CS／HDMIの絶対選択は、HomeKitの既存の入力選択画面から利用できます。最初の決定で設定した番組表／メニューを開くと、矢印は通常の方向キー、決定は通常の決定キーへ切り替わります。
