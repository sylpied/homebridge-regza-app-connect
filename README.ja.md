...
v0.6.1では、Homebridgeの専用設定画面で必須項目を先に表示し、接続・電源・リモコン・入力設定を折りたたんで整理しています。`55J10X`モデルプロファイルを選び、IPアドレスとApp Connectのユーザー名/パスワードだけ入力すれば使えます。
...
消音状態は`GET /v2/remote/status/mute`で同期します。通常の入力・消音取得はすべての確認方式で既定30秒のまま継続します。定期取得した再生状態が地デジ・BS・CSの`broadcast`なら、テレビを確実にONと判定して補正します。既定の`operation`（操作連動）では、Homebridge起動時に1回だけ消音プローブを実行し、その後は定期的な音切れを発生させません。HomeKitの電源操作、確認できた地デジ・BS・CS切替、成功したMute操作からも記憶状態を更新します。HDMIとスタンバイのずれを定期補正したい場合だけ`interval`を、消音プローブを完全に無効化する場合は`optimistic`を選択します。内蔵アプリ表示中の状態はまだ十分に検証できていません。
