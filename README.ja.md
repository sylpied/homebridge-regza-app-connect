...
- 地デジ・BS・CS切り替えに対応
- 情報ボタンでスタートメニューを開き、決定は常に通常の決定キーとして動作
- 再生／一時停止ボタンは専用の再生・一時停止キーを交互に送り、その後に日付選択用の緑キーを送信
...
## HomeKitリモコンのナビゲーションとPlay/Pause

HomeKitのTVリモコンにはREGZA専用の番組表ボタンや色ボタンがありません。「情報」で設定した番組表／メニューを開き、「決定」は常に通常の決定キーとして送信します。
...
- `guideFirst`：情報ボタンで番組表`40BF6E`
- `menuFirst`：情報ボタンで設定メニュー（TVは`40BFD0`、DBR-M590は`46`）
- `quickFirst`：情報ボタンでクイックメニュー`40BF27`
- `normal`：情報ボタンで画面表示／情報を送信（決定は常に通常の決定）

通常視聴中は、上／下でチャンネルUP／DOWN、右で「地デジ→BS→CS→地デジ」、左で逆順に切り替えます。HDMI表示中の最初の切替は地デジへ戻します。地デジ／BS／CS／HDMIの絶対選択は、HomeKitの既存の入力選択画面から利用できます。情報ボタンで設定した番組表／メニューを開くと、矢印は通常の方向キーへ切り替わります。

Play/Pauseは通常の一時停止／再生キーを交互に送った後、短い待機を挟んで色キーも送ります。検証済みプロファイルでは55J10Xが青、DBR-M590が緑です。通常再生中は色キーが原則無視され、番組表／タイムシフト画面では日付選択を開けるため、プラグインが現在のメニュー階層を推測する必要がありません。

REGZAテレビでは、番組表／メニュー内で決定した後、矢印または決定が15秒間押されなければ「戻る」を自動送信し、画面を閉じてナビゲーションモードを解除します。追加操作があれば15秒を再計測します。情報ボタンで番組表を再度開けるため、決定の役割は変わりません。レコーダーでは再生を中断しないよう「戻る」を自動送信せず、プラグイン内部のナビゲーション状態だけを解除します。
